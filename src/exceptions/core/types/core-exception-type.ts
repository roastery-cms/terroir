 ...
 * originates from.
 *
 * Stored on every exception under the {@link Layer} symbol, this value is the
 * runtime discriminator used by error-handling middleware, logging adapters
 * and HTTP layers to decide how an error should be reported, mapped to a
 * status code, or surfaced to the caller.
 *
 * @remarks
 ...
 * | `"application"`| Use-case / orchestration failures, one per HTTP error status (bad request, unauthorized, forbidden, too many requests). |
 ...
 * @see {@link Layer} — symbol key under which a value of this type is stored.
 */
